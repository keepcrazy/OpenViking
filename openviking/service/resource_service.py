...
class ResourceService:
    ...
    async def add_resource(
        ...
        **kwargs,
        ...
        if self._should_use_connector(path):
    ...
    def _should_use_connector(self, path: str) -> bool:
        ...
        if not config.enable:
            return False
        if not isinstance(path, str) or not path.startswith("tos://"):
            return False
        if "tos" not in config.allowed_add_types:
            logger.info(
                "[ResourceService] add type 'tos' inferred from path is not in "
    ...
    async def _add_resource_via_connector(
        ...
        **kwargs: Any,
        ...
        if watch_interval > 0:
            raise InvalidArgumentError("Connector imports do not support watch_interval yet.")
        ...
        task_resource_id = target.to or None
        ...
        reserved = {"path_prefix", "include_child"}
        for k, v in connector_args.items():
            if k not in reserved:
                extra_params[k] = v

        tos_path = path[len("tos://") :].strip()
        if not tos_path:
            raise InvalidArgumentError(
                "Connector TOS import requires path='tos://<bucket>/<prefix>'."
            )
        ...
        if not connector_task_key:
            raise InternalError(f"Connector accepted the import but returned no task key: {result}")
