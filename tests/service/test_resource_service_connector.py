...
@pytest.mark.asyncio
async def test_add_resource_routes_tos_path_to_connector(
    monkeypatch,
    resource_service,
    resource_processor,
    request_context,
    ...
    assert connector.account_id == request_context.account_id
    assert connector.submit_calls == [
        {
            "add_type": "tos",
            "api_key": "sk-test",
            "tos_path": "bucket/input",
            "path_prefix": ["docs"],
            "include_child": False,
...
@pytest.mark.asyncio
async def test_add_resource_uses_standard_pipeline_for_non_tos_path(
    monkeypatch,
    resource_service,
    resource_processor,
    request_context,
    ...
    assert resource_processor.calls[0]["parser"] == "web"
...
@pytest.mark.asyncio
async def test_connector_derives_tos_path_and_leaves_unknown_task_resource_unset(
    monkeypatch,
    resource_service,
    request_context,
):
    ...
    submit = RecordingConnectorClient.instances[0].submit_calls[0]
    assert "resource_id" not in submit
    assert submit["tos_path"] == "bucket/input"
    assert tracker.created[0]["resource_id"] is None


@pytest.mark.asyncio
async def test_connector_callback_without_add_type_uses_standard_pipeline(
    monkeypatch,
    resource_service,
    resource_processor,
    request_context,
    ...
    result = await resource_service.add_resource(
        path="https://example.com/connector-output.md",
        ctx=request_context,
        to="viking://resources/callback-output",
    )

    assert result["root_uri"] == "viking://resources/callback-output"
    assert len(resource_processor.calls) == 1


@pytest.mark.asyncio
async def test_connector_watch_is_rejected(
    monkeypatch,
    resource_service,
    request_context,
):
    ...
    with pytest.raises(InvalidArgumentError, match="do not support watch_interval"):
...
async def test_monitor_connector_task_completes_succeeded_status(monkeypatch, request_context):
    ...
    async def get_task_info(task_key, api_key):
...
async def test_monitor_connector_task_fails_terminal_error_status(monkeypatch, request_context):
    ...
    async def get_task_info(_task_key, api_key):
        assert api_key == "sk-test"
